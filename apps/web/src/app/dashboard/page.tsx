  ...
  return (
    <div>
      ...
      <div style={{ marginTop: '2rem', display: 'grid', gap: '1rem', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))' }}>
        <div style={{ padding: '1.5rem', backgroundColor: 'var(--bg-elevated)', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border-color)' }}>
          <h3 style={{ fontWeight: 600, marginBottom: '0.5rem' }}>Моя Организация</h3>
          {user?.organizationId ? (
            <p style={{ color: 'var(--success)' }}>Профиль заполнен</p>
          ) : (
            <p style={{ color: 'var(--warning)' }}>
              Пожалуйста, <Link href="/dashboard/settings" style={{ textDecoration: 'underline' }}>заполните профиль организации в настройках</Link>.
            </p>
          )}
        </div>
      </div>
